...
export function initCheckout() {
    ...
    // ----------------------------------------
    // 3. 純粋なバリデーション（DOM操作なし）
    //    フィールドの検証結果を { id, label, isValid } の配列で返す
    // ----------------------------------------
    const validateFields = (fields) => {
        return fields.map(({ id, pattern, label }) => {
            const field = document.getElementById(id);
            if (!field) return { id, label, isValid: false };
            const value   = field.value.trim();
            const isValid = value !== '' && (!pattern || pattern.test(value));
            return { id, label, isValid };
        });
    };

    // ----------------------------------------
    // 4. バリデーション結果を受け取ってDOMに反映（エラークラスの付与）
    // ----------------------------------------
    const applyErrorStyles = (results) => {
    ...
    // ----------------------------------------
    // 5. バリデーション結果をもとにUIを更新する関数
    // ----------------------------------------
    const updateUI = () => {
        const selectResults = validateFields(selectProductFields);
        const userResults   = validateFields(userInfoFields);

        // エラースタイルをDOMへ反映
        applyErrorStyles(selectResults);
        applyErrorStyles(userResults);

        // 未入力・不正ラベルの抽出
        const selectMissing = selectResults.filter(r => !r.isValid).map(r => r.label);
        const userMissing   = userResults.filter(r => !r.isValid).map(r => r.label);
        ...
        if (submitBtn) {
            ...
            submitBtn.classList.toggle('cta-btn--disabled', !canSubmit);
        }
    ...
    // 7. submitイベント（全条件通過時のみアラート）
    // ※ デモ実装のため preventDefault() で送信を抑止
